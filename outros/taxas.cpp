...
double apply_discount(double before_discount, double discount) {
    ...
    double desconto = before_discount  * (1.0 - (discount / 100.0));
    cout <<"valor: "<< before_discount << " com desconto de" << discount << "%: "<< desconto << endl;
    return desconto;
}
...
int monthly_rate(double hourly_rate, double discount) {
    ...
    cout << "Dia: " << dia << endl;
    double mes = dia * 22.0;
    cout << "Mensal:" << mes << endl;
    return std::ceil(apply_discount(mes, discount));
}
...
int main() {
   cout << "Calculo Taxa mensal: "<< endl << monthly_rate(62, 0.0) << endl;   
}
